...
{
    FluidCube *cube = static_cast<FluidCube*>(malloc(sizeof(FluidCube)));
    int N = size;
...
int main( int argc, char** argv)
{
	...
	int size = parser.addOption<int>("-n", 100, "Size of simulation domain");
	int maxIter = parser.addOption<int>("-i", 100, "Maximum number of simulation iteration");
	int diff = parser.addOption<int>("-d", 1, "Diffusion coefficient");
	int visc = parser.addOption<int>("-v", 1, "Viscosity coefficient");

	// allocation
	FluidCube* fc = FluidCubeCreate(size, diff, visc, 1.0);
	...
	{
		FluidCubeAddDensity(fc,i,j,k,0);
		FluidCubeAddVelocity(fc,i,j,k,0,0,0);
	}
	
	// perturbation
	FluidCubeAddDensity(fc,50,50,50,255);
	FluidCubeAddVelocity(fc,50,50,50,50,0,0);
